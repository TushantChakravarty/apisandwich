...
const processTransactionTest =async  ()=>{
  ...
  const processPayinRequestBazorpay =async  (data)=>{
   ...
   const response = await fetch(`https://api.bazorpay.com/transactions/createtransaction?merchant_id=MR_0000013&api_key=CwZYdjcDNFlkY56P4H5SEvhCyhFE05KidHhYPW_yo9GzXSQ1&transaction_id=${txId}&amount=${data.amount}&user_name=${data.username}&mobile_no=${data.phone}&email=${data.customer_email}&platform=ios`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Access-Control-Allow-Origin' : '*',
