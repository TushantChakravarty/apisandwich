    ...
    if (req.body.all === false) {
      ...
      let salesAggregate = await Sales.aggregate([
        ...
        { $unwind: "$merchants" },
        {
          $group: {
            _id: null,
            totalLast24hr: { $sum: "$merchants.last24hr" },
            totalLast24hrSuccess: { $sum: "$merchants.last24hrSuccess" },
            totalLast24hrTotal: { $sum: "$merchants.last24hrTotal" },
            totalYesterday: { $sum: "$merchants.yesterday" },
            totalYesterdayTransactions: { $sum: "$merchants.yesterdayTransactions" },
            totalSuccessfulTransactions: { $sum: "$merchants.successfulTransactions" },
            totalTotalTransactions: { $sum: "$merchants.totalTransactions" }
          }
        }
      ]);
