...
async function getTransactionsByStatus(details) {
  ...
  const transactions = user.transactions
    .reverse()
    .filter((t) => t.status === details.status);
...
async function getAllTransactionWithSuccessStatus(emailId, details) {
  ...
  const paginated = Array.from(aggregatedTransactions.values()).slice(
    details.skip,
    details.skip + details.limit
  );
        ...
        if (user.transactions && user.transactions.length > 0) {
          user.transactions.forEach((transaction) => {
            ...
            if (
              ...
              transaction.gateway &&
              status === transaction.status &&
              gatewayName=='phonepe'? gatewayName == transaction.gateway ||transaction.gateway =="phonepeCollect":  gatewayName == transaction.gateway
            ) {
              vol += Number(transaction.amount);
            }
          });
...
async function getTotalGatewayBalance(hash) {
  ...
  Users.forEach((user) => {
    const userTransactions = user.transactions.filter((t) => {
      // Perform date comparison
      return t.status == "success" && t.hash == hash;
    });
    userTransactions.map((item) => {
      totalVolume = Number(totalVolume) + Number(item.amount);
    });
...
async function getTotalMerechantBalance(emailId,status) {
  try {
    const User = await usrDao.findOne({
      emailId: emailId,
    });
    ...
    User.settlements.forEach(async (transaction) => {
      try {
          ...
          totalSettlements += Number(transaction.amount);
    ...
    let balance = totalVolume-totalSettlements
    //console.log('Total volume for Merchant:',emailId, totalVolume,totalSettlements,balance);
    return balance;
  } catch (error) {
    console.error("Error in getTotalVolumeMerchant:", error.message);
    throw error; // rethrow the error for further handling
    ...
    Users.forEach(async (user) => {
      try {
        ...
        if (user.transactions && user.transactions.length > 0) {
            ...
            vol = await getTotalMerechantBalance(user.emailId,'success')
...
async function getTotalAdminBalance(emailId, status) {
  try {
    const user = await usrDao.findOne({
      emailId: emailId,
    });
  ...
  try {
    ...
    for (const user of users) {
      try {
        ...
        if (user.transactions && user.transactions.length > 0) {
          vol = await getTotalAdminBalance(user.emailId, 'success');
          totalBalance += vol;
        } else {
