...
async function updateGatewaySettlements(query, updateDetails) {
  ...
  return adminDao.findOneAndUpdate(query, {$push:{gatewaySettlements:updateDetails}},{safe: true, upsert: true, new : true})
...
const getAllGatewaysSettlements = async (details) => {
  ...
  const paginated = allTx.reverse().slice(details.skip, details.skip + details.limit);
  ...
  try {
    ...
    if (user.transactions && user.transactions.length > 0) {
      user.transactions.forEach((transaction) => {
        ...
        const formattedTransactionDate = moment.tz(transactionDate, 'Asia/Kolkata');
  ...
  try {
    ...
    Users.forEach(async (user) => {
      try {
        ...
        console.error(`Error updating profile for ${user.emailId}: ${updateError.message}`);
      }
    });
...
async function updateGatewayVolumeData() {
  try {
   ...
   gateways.map(async (item)=>{
     let total = 0
     let totalVolume = await getTotalVolumeGateway('success',item.gatewayName)
     total = totalVolume
     if(item.gatewayName == 'paythrough')
     {
      totalVolume = await getTotalVolumeGateway('success','paythroughIntent')
      total += totalVolume
     }
