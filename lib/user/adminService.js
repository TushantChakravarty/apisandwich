                ...
                for (const user of response) {
                    ...
                    for (let i = 0; i < user.transactions.length; i++) {
                        ...
                        const body = {
                            ...
                            gateway: transaction.hash == 'xyzAirpay' ? 'airpay' : transaction.hash == 'xyzPaythrough' ? 'paythrough' : transaction.hash == 'xyzbazorpay' ? 'bazarpay' : transaction.hash == 'xyzSwipeline' ? "swipeline" : ''
                        }
        ...
        return await validateRequest(details)
            .then(async (response) => {
                if (response == true) {
                    ...
                    if (response)
                        return mapper.responseMappingWithData(usrConst.CODE.Success, usrConst.MESSAGE.Success, {isBanned:response.isBanned,settlements:paginatedTransactions})
                    else
                        return mapper.responseMapping(usrConst.CODE.BadRequest, 'Invalid details')
                } else if (response == false) {
                    return mapper.responseMapping(usrConst.CODE.FRBDN, 'Invalid apiKey')
