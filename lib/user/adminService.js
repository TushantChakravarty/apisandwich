...
const { updateTransactionsData, updateTransactionStatus, getAllTransactions, getDataById, getTransactionById } = require('./transactionDao')
...
async function saveTxPaythrough(details) {
    ...
    if (details) {
        ...
        const transaction = await getTransactionById(details.transaction_id)//dao.getTransactionDetails(details.transaction_id)
...
async function saveTxAirpay(details) {
    ...
    if (details) {
        ...
        const transaction = await getTransactionById(details.APTRANSACTIONID)
        // console.log(transaction)
        const admin = await dao.getUserDetails(adminQuery)
        ...
        updateTransactionStatus( details.APTRANSACTIONID, updateObj)
...
async function saveTxSwipeline(details) {
    ...
    if (details) {
        ...
        const transaction = await getTransactionById(decryptedData.txnId)
        // console.log(transaction)
        const admin = await dao.getUserDetails(adminQuery)
...
//   }
    ...
    if (details) {
        ...
        const transaction = await getTransactionById(decoded.data.merchantTransactionId)
        // console.log(transaction)
        const admin = await dao.getUserDetails(adminQuery)
        ...
        .then((response) => {
            if (response == true) {
                ...
                if (details.email_Id == 'all') {
                    return transactionDao.getTransactionsByDate(details.start_date, details.end_date).then((user) => {
                        if (user) {
                            //console.log('success', user)
                    ...
                    return transactionDao.getTransactionsByDateAndStatus(details.start_date, details.end_date, details.status).then((user) => {
                        if (user) {
                            //console.log('success', user)
    ...
    try {
        if (details.emailId && details.apiKey) {
            ...
            if (isValidRequest) {
                ...
                return mapper.responseMappingWithData(usrConst.CODE.Success, usrConst.MESSAGE.Success, allTx);
            } else {
                return mapper.responseMapping(usrConst.CODE.FRBDN, 'Invalid apiKey');
            }
        } else {
