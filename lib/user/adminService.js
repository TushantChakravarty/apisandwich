...
//   }
    ...
    if (details) {
        ...
        let updateObj = {
            ...
            utr: decoded.data.paymentInstruction.utr,
        ...
        if (decoded.data.responseCode.toLowerCase() == 'success') {
            dao.getUserBalance2(query)
                .then((response) => {
                    ...
                    const adminBalance = admin.balance
                    let adminUpdate = {
                        last24hr: Number(admin24hr) + Number(decoded.data.paymentInstruction.amount),
                        balance: Number(adminBalance) + Number(decoded.data.paymentInstruction.amount),
                        totalTransactions: Number(admin.totalTransactions) + 1,
                        successfulTransactions: Number(admin.successfulTransactions) + 1,
                        last24hrSuccess: Number(admin.last24hrSuccess) + 1,
                        last24hrTotal: Number(admin.last24hrTotal) + 1,
                    }
                    const feeCollected = Number(gatewayData.feeCollected24hr) + (Number(response[0].platformFee) > 0 ? (Number(decoded.data.paymentInstruction.amount) * (Number(response[0].platformFee) / 100)) : 0);
                    const totalFeeCollected = Number(gatewayData.totalFeeCollected) + (Number(response[0].platformFee) > 0 ? (Number(decoded.data.paymentInstruction.amount) * (Number(response[0].platformFee) / 100)) : 0)
                    console.log(feeCollected, totalFeeCollected)
                    let gatewayUpdate = {
                        last24hr: Number(gatewayData.last24hr) + Number(decoded.data.paymentInstruction.amount),
                        last24hrSuccess: Number(gatewayData.last24hrSuccess) + 1,
                        successfulTransactions: Number(gatewayData.successfulTransactions) + 1,
                        totalVolume: Number(gatewayData.totalVolume) + Number(decoded.data.paymentInstruction.amount),
                        feeCollected24hr: feeCollected,
                        totalFeeCollected: totalFeeCollected,
                    }
                    console.log('gateway update', gatewayUpdate)
                    ...
                    let updateObj = {
                        balance: Number(decoded.data.paymentInstruction.amount) + Number(balance),
                        utr: decoded.data.paymentInstruction.utr,
                        last24hr: Number(user24hr) + Number(decoded.data.paymentInstruction.amount),
                        totalTransactions: Number(response[0].totalTransactions) + 1,
                        successfulTransactions: Number(response[0].successfulTransactions) + 1,
                        last24hrSuccess: Number(response[0].last24hrSuccess) + 1,
                        last24hrTotal: Number(response[0].last24hrTotal) + 1,
                        todayFee: Number(response[0].platformFee) > 0 ?Number(response[0].todayFee) + (Number(decoded.data.paymentInstruction.amount) * (Number(response[0].platformFee) / 100)) : 0
                    }
                    console.log("updateObj", updateObj)
                    const txData = {
                        transaction_id:transaction.transactionId,
                    ...
                    let callBackDetails = {
                        ...
                        status: decoded.data.responseCode.toLowerCase(),
                        amount: decoded.data.paymentInstruction.amount,
                        utr: decoded.data.paymentInstruction.utr,
                        date: transaction.transaction_date,
                        encryptedData:encryptedData
                    }
                ...
                if (response == true) {
                    ...
                    for (const user of response) {
                        const successfulTransactions = {
                            business_name: user.business_name,
                            email_id: user.emailId,
                            todayVolume: user.last24hr,
                            todayTransactions: user.last24hrTotal,
                            yesterdayVolume: user.yesterday,
                            yesterdayTransactions:user.yesterdayTransactions,
                            walletBalance: user.balance,
                            todayFee: user.todayFee,
                            yesterdayFee: user.yesterdayFee
                        }
                        //.filter(transaction => transaction.status === 'success');
                        allTransactions.push(successfulTransactions);
                    }
