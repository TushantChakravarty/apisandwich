...
async function addSales(req, res) {
  try {
    const { email_Id, password, merchant_id } = req.body
    ...
    if (isExistUser) {
      res.send({
        CODE: 404,
        MESSAGE: "This salesman is already registered."
      })
    }
    else {
      ...
      if (existingMerchants.length !== merchant_id.length) {
        return res.send({
          CODE: 404,
          MESSAGE: "Not all merchant id exist in the database"
        });
      }

      console.log("coming to this")

      // Update the 'assigned' field to true for all existing merchants
      ...
      for (let i = 0; i < existingMerchants.length; i++) {
        existingMerchants[i].assigned = true
        existingMerchants[i].assignedTo = salesman._id
...
async function allMerchants(req, res) {
  try {
    ...
    const selectedFields = ['_id', 'emailId', 'business_name', 'assigned'];
