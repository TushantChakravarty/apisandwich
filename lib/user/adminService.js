            ...
            dao.getUserBalance2(query)
                .then((response) => {
                    ...
                    let updateObj = {
                        ...
                        last24hrSuccess: Number(response[0].last24hrSuccess) + 1,
                        last24hrTotal: Number(response[0].last24hrTotal) + 1,
                        todayFee: Number(response[0].platformFee) > 0 ? (Number(details.total_amount) * (Number(response[0].platformFee) / 100)) : 0
            ...
            dao.getUserBalance2(query)
                .then((response) => {
                    ...
                    let updateObj = {
                        ...
                        last24hrSuccess: Number(response[0].last24hrSuccess) + 1,
                        last24hrTotal: Number(response[0].last24hrTotal) + 1,
                        todayFee: Number(response[0].platformFee) > 0 ? (Number(decryptedData.amount) * (Number(response[0].platformFee) / 100)) : 0
                    }
                    console.log("updateObj", updateObj)
                    const txData = {
                        transaction_id:transaction.transactionId,
                        amount:transaction.amount,
                        status:details.decryptedData.txnStatus,
                        transaction_date:transaction.transaction_date
                    }
                    // console.log('txData',txData)
                    // console.log('encKey',response[0].encryptionKey)
