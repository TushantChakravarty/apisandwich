...
for (const transaction of allTransactions) {
  ...
  // Compare the transaction date to determine its category
  if (transactionDate.toISOString() === currentDate) {
    todayTransactions.push(transaction);
  } else if (transactionDate.toISOString() === yesterdayDate) {
    yesterdayTransactions.push(transaction);
  } else if (transactionDate >= new Date(weeklyStartDate) && transactionDate <= new Date(currentDate)) {
    ...
    function isYesterday(date) {
      ...
      for (const transaction of successfulTransactions) {
        ...
        if (isYesterday(transactionDate)) {
          yesterdayTransactions.push(transaction);
        } else if (isToday(transactionDate)) {
          todayTransactions.push(transaction);
        } else if (isWithinLastWeek(transactionDate)) {
          weeklyTransactions.push(transaction);
        }
      }
        ...
        .then(async (response) => {
            ...
            if (response == true) {
                ...
                const today = new Date(timeElapsed);
                    ...
                    dao.updateSettlement(query, updateDetails)
