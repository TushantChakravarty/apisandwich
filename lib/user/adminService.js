...
const { decryptData } = require('../controllers/swipeline')
                ...
                if (response == true) {
                    ...
                    response?.map((response)=>{
                        ...
                        const Data ={
                            ...
                            payinGateway:response.gateway,
                            payoutGateway:response.payoutGateway,
                            balance:response.balance,
                            platformFee:response.platformFee,
                            business_name:response.business_name,
                            is_Banned:response.isBanned
                            
                        }
                        respData.push(Data)
                    })
                    return mapper.responseMappingWithData(usrConst.CODE.Success, usrConst.MESSAGE.Success, respData)
        ...
        return await validateRequest(details)
            .then(async (response) => {
                if (response == true) {
                    ...
                    return mapper.responseMappingWithData(usrConst.CODE.Success, usrConst.MESSAGE.Success, admin.balance)
    ...
    return await validateRequest(details)
        .then(async (response) => {
            ...
            if (response == true) {
                ...
                const adminProfit = user.platformFee > 0 ? (Number(details.amount) * (Number(user.platformFee) / 100)) : 0
                ...
                const updateDetails = {
                    ...
                    notes: details.notes,
                    feeCharged: adminProfit,
                    amountSettled: Number(details.amount)-Number(adminProfit), // user.platformFee > 0 ? Number(details.amount) - (Number(details.amount) * (Number(user.platformFee) / 100)) : Number(details.amount)
                    usdt:details.usdt
                }
                ...
                if (Number(admin.balance) < Number(details.amount)) {
                    return mapper.responseMappingWithData(usrConst.CODE.BadRequest, "Low merchant balance", 'Low Admin balance')
                }
                        ...
                        if (response) {
                            return dao.getUserBalance(query)
                                .then((response) => {
                                    ...
                                    const adminProfit = response.platformFee > 0 ? (Number(details.amount) * (Number(response.platformFee) / 100)) : 0
        ...
        if (details.amount && details.status.toLowerCase() == 'success') {
            dao.getUserBalance2(query)
                .then(async (response) => {
                    ...
                    console.log(balance)
                    const feeCollected = Number(gatewayData.feeCollected24hr) + (Number(response[0].platformFee) > 0 ? (Number(details.amount) * (Number(response[0].platformFee) / 100)) : 0)
                    const totalFeeCollected = Number(gatewayData.totalFeeCollected) + (Number(response[0].platformFee) > 0 ? (Number(details.amount) * (Number(response[0].platformFee) / 100)) : 0)
                    ...
                    let updateObj = {
                        ...
                        last24hrSuccess: Number(response[0].last24hrSuccess) + 1,
                        last24hrTotal: Number(response[0].last24hrTotal) + 1,
                        todayFee: Number(response[0].platformFee) > 0 ? (Number(details.amount) * (Number(response[0].platformFee) / 100)) : 0
                    ...
                    await callbackPayin(callBackDetails, response[0].callbackUrl)
        ...
        if (details.total_amount && details.status == 'success') {
            dao.getUserBalance2(query)
                .then(async (response) => {
                    ...
                    const feeCollected = Number(gatewayData.feeCollected24hr) + (Number(response[0].platformFee) > 0 ? (Number(details.total_amount) * (Number(response[0].platformFee) / 100)) : 0)
                    const totalFeeCollected = Number(gatewayData.totalFeeCollected) + (Number(response[0].platformFee) > 0 ? (Number(details.total_amount) * (Number(response[0].platformFee) / 100)) : 0)
                    let gatewayUpdate = {
                    ...
                    let updateObj = {
                        ...
                        last24hrSuccess: Number(response[0].last24hrSuccess) + 1,
                        last24hrTotal: Number(response[0].last24hrTotal) + 1,
                        todayFee: Number(response[0].platformFee) > 0 ?Number(response[0].todayFee) + (Number(details.total_amount) * (Number(response[0].platformFee) / 100)) : 0
...
//                     let adminUpdate = {
//                         last24hr: Number(admin.last24hr) + Number(details.total_amount),
//                         balance: Number(admin.balance) + Number(details.total_amount),
//                         totalTransactions: Number(admin.totalTransactions) + 1,
//                         successfulTransactions: Number(admin.successfulTransactions) + 1,
//                         last24hrSuccess: Number(admin.last24hrSuccess) + 1,
//                         last24hrTotal: Number(admin.last24hrTotal) + 1
...
//                     await dao.updateProfile(adminQuery, adminUpdate);
//                     await dao.updateUserProfile2(query, updateObj);
//                     await dao.updateGatewayDetails('paythrough', gatewayUpdate);
//                     await callbackPayin(callBackDetails, response[0].callbackUrl);
//                     await dao.updateTransactions(query, details.transaction_id, UpdateObj);
//                 } else {
//                     const response = await dao.getUserBalance2(query);
...
//                     UpdateObj.totalTransactions = response[0].totalTransactions + 1;
//                     UpdateObj.last24hrTotal = response[0].last24hrTotal + 1;
...
//         await enqueueUpdate(updateTask);
...
//old
// async function saveTxPaythrough1(details) {
//     if (!details) {
//         return mapper.responseMapping(usrConst.CODE.BadRequest, 'invalid details');
//     }
...
//             const feeCollected = Number(gatewayData.feeCollected24hr) + (
//                 Number(response[0].platformFee) > 0 ?
//                 (Number(details.total_amount) * (Number(response[0].platformFee) / 100)) : 0
//             );

//             const totalFeeCollected = Number(gatewayData.totalFeeCollected) + (
//                 Number(response[0].platformFee) > 0 ?
//                 (Number(details.total_amount) * (Number(response[0].platformFee) / 100)) : 0
//             );
...
//             updateObj.balance = Number(details.total_amount) + Number(balance);
//             updateObj.last24hr = Number(user24hr) + Number(details.total_amount);
//             updateObj.totalTransactions = Number(response[0].totalTransactions) + 1;
//             updateObj.successfulTransactions = Number(response[0].successfulTransactions) + 1;
//             updateObj.last24hrSuccess = Number(response[0].last24hrSuccess) + 1;
//             updateObj.last24hrTotal = Number(response[0].last24hrTotal) + 1;
//             updateObj.todayFee = Number(response[0].platformFee) > 0 ?
//                 Number(response[0].todayFee) + (Number(details.total_amount) * (Number(response[0].platformFee) / 100)) : 0;
...
//         const userUpdated = await dao.updateTransactions(query, details.transaction_id, updateObj);
            ...
            dao.getUserBalance2(query)
                .then(async (response) => {
                    ...
                    let updateObj = {
                        ...
                        last24hrSuccess: Number(response[0].last24hrSuccess) + 1,
                        last24hrTotal: Number(response[0].last24hrTotal) + 1,
                        todayFee: Number(response[0].platformFee) > 0 ?Number(response[0].todayFee) + (Number(details.AMOUNT) * (Number(response[0].platformFee) / 100)) : 0
                    }
...
//                     let adminUpdate = {
//                         last24hr: Number(admin.last24hr) + Number(details.AMOUNT),
//                         balance: Number(admin.balance) + Number(details.AMOUNT),
//                         totalTransactions: Number(admin.totalTransactions) + 1,
//                         successfulTransactions: Number(admin.successfulTransactions) + 1,
//                         last24hrSuccess: Number(admin.last24hrSuccess) + 1,
...
//                     await dao.updateGatewayDetails('airpay', gatewayUpdate);
//                     await callbackPayin(callBackDetails, response[0].callbackUrl);
//                     await dao.updateTransactions(query, details.APTRANSACTIONID, UpdateObj);
//                 } else {
//                     const response = await dao.getUserBalance2(query);
...
//             updateObj.balance = Number(details.AMOUNT) + Number(balance);
//             updateObj.last24hr = Number(user24hr) + Number(details.AMOUNT);
//             updateObj.totalTransactions = Number(response[0].totalTransactions) + 1;
//             updateObj.successfulTransactions = Number(response[0].successfulTransactions) + 1;
//             updateObj.last24hrSuccess = Number(response[0].last24hrSuccess) + 1;
//             updateObj.last24hrTotal = Number(response[0].last24hrTotal) + 1;
//             updateObj.todayFee = Number(response[0].platformFee) > 0 ?
//                 Number(response[0].todayFee) + (Number(details.AMOUNT) * (Number(response[0].platformFee) / 100)) : 0;
...
//         const userUpdated = await dao.updateTransactions(query, details.APTRANSACTIONID, updateObj);
...
async function saveTxSwipeline(details) {
    ...
    if (details) {
        ...
        const transaction = await dao.getTransactionDetails(decryptedData.txnId)
        // console.log(transaction)
        const admin = await dao.getUserDetails(adminQuery)
        ...
        if (decryptedData.amount && decryptedData.txnStatus == 'success') {
            dao.getUserBalance2(query)
                .then((response) => {
                    ...
                    const feeCollected = Number(gatewayData.feeCollected24hr) + (Number(response[0].platformFee) > 0 ? (Number(decryptedData.amount) * (Number(response[0].platformFee) / 100)) : 0);
                    const totalFeeCollected = Number(gatewayData.totalFeeCollected) + (Number(response[0].platformFee) > 0 ? (Number(decryptedData.amount) * (Number(response[0].platformFee) / 100)) : 0)
                    console.log(feeCollected, totalFeeCollected)
                    let gatewayUpdate = {
                        last24hr: Number(gatewayData.last24hr) + Number(decryptedData.amount),
                        last24hrSuccess: Number(gatewayData.last24hrSuccess) + 1,
                        successfulTransactions: Number(gatewayData.successfulTransactions) + 1,
                        totalVolume: Number(gatewayData.totalVolume) + Number(decryptedData.amount),
                        feeCollected24hr: feeCollected,
                        totalFeeCollected: totalFeeCollected,
                    }
                    console.log('gateway update', gatewayUpdate)
                    ...
                    let updateObj = {
                        ...
                        last24hrSuccess: Number(response[0].last24hrSuccess) + 1,
                        last24hrTotal: Number(response[0].last24hrTotal) + 1,
                        todayFee: Number(response[0].platformFee) > 0 ?Number(response[0].todayFee) + (Number(decryptedData.amount) * (Number(response[0].platformFee) / 100)) : 0
                    }
                    console.log("updateObj", updateObj)
                    const txData = {
                        transaction_id:transaction.transactionId,
        ...
        return dao.updateTransactions(query, decryptedData.txnId, updateObj).then((userUpdated) => {
            if (userUpdated) {
                //console.log('success', userUpdated)
...
async function saveTxPhonepe(details) {
    ...
    const decodedString = Buffer.from(details.response, 'base64').toString('utf-8');
...
//   }
    ...
    if (details) {
        ...
        const transaction = await dao.getTransactionDetails(decoded.data.merchantTransactionId)
        // console.log(transaction)
        const admin = await dao.getUserDetails(adminQuery)
        ...
        if (decoded.data.responseCode.toLowerCase() == 'success') {
            dao.getUserBalance2(query)
                .then((response) => {
                    ...
                    const feeCollected = Number(gatewayData.feeCollected24hr) + (Number(response[0].platformFee) > 0 ? (Number(amount) * (Number(response[0].platformFee) / 100)) : 0);
                    const totalFeeCollected = Number(gatewayData.totalFeeCollected) + (Number(response[0].platformFee) > 0 ? (Number(amount) * (Number(response[0].platformFee) / 100)) : 0)
                    console.log(feeCollected, totalFeeCollected)
                    let gatewayUpdate = {
                    ...
                    let updateObj = {
                        ...
                        last24hrSuccess: Number(response[0].last24hrSuccess) + 1,
                        last24hrTotal: Number(response[0].last24hrTotal) + 1,
                        todayFee: Number(response[0].platformFee) > 0 ?Number(response[0].todayFee) + (Number(amount) * (Number(response[0].platformFee) / 100)) : 0
                    }
                    console.log("updateObj", updateObj)
                    const txData = {
                        transaction_id:transaction.transactionId,
            ...
            dao.getUserBalance2(query)
                .then((response) => {
                    ...
                    let callBackDetails = {
                        ...
                        status: decoded.data.responseCode.toLowerCase(),
                        amount: amount,
                        utr: decoded.data.paymentInstrument.utr?decoded.data.paymentInstrument.utr:'',
                        date: transaction.transaction_date,
                        encryptedData:encryptedData
                    }
        ...
        return dao.updateTransactions(query, decoded.data.merchantTransactionId, updateObj).then((userUpdated) => {
            if (userUpdated) {
                //console.log('success', userUpdated)
        ...
        .then((response) => {
            if (response == true) {
                ...
                if (details.email_Id == 'all') {
                    return dao.getAllTransactionByDate(details.start_date, details.end_date).then((user) => {
                        if (user) {
                            //console.log('success', user)
                    ...
                    return dao.getAllTransactionByDateWithStatus(details.start_date, details.end_date, details.status).then((user) => {
                        if (user) {
                            //console.log('success', user)
                    ...
                    return dao.getTransactionByDateWithStatus(query, details.start_date, details.end_date, details.status).then((user) => {
                        if (user) {
                            //console.log('success', user)
            ...
            .then(async (response) => {
                if (response == true) {
                    ...
                    for (let i = 0; i < response.length; i++) {
                        ...
                        for (let j = 0; j < successfulTransactions.length; j++) {
                            const item = successfulTransactions[j];
                            const gateway =
                                item.hash === 'xyzAirpay'
                                    ? 'airpay'
                                    : item.hash === 'xyzPaythrough'
                                        ? 'paythrough'
                                        : item.hash === 'xyzbazorpay'
                                            ? 'bazarpay'
                                            : item.hash === 'xyzSwipeline'
                                                ? "swipeline"
                                                : '';
                            ...
                            const transaction = {
                                ...
                                "amount": item.amount,
                                "currency": item.currency, // Corrected typo
                    ...
                    for (let i = 0; i < allTransactions.length; i++) {
                        ...
                        if (allTransactions[i].utr == details.utr || allTransactions[i].transactionId == details.utr) {
...
async function getAllTransactionsByStatus(details) {
    return validateRequest(details)
        .then((response) => {
            if (response == true) {
                return dao.getAllTransactionWithStatus(details.status, details).then((user) => {
                    if (user) {
                        //console.log('success', user)
...
async function getMerchantLogs(details) {
    return validateRequest(details)
        .then((response) => {
            if (response == true) {
                return dao.getAllTransactionWithSuccessStatus(details.email_Id,details).then((userTransactions) => {
                    if (userTransactions) {
                        //console.log('success', user)

                        return mapper.responseMappingWithData(usrConst.CODE.Success, usrConst.MESSAGE.Success, userTransactions)
...
async function getMerchantTotalSettlementsAndVolume(details) {
    return validateRequest(details)
        .then((response) => {
            if (response == true) {
                return dao.getTotalVolumeAndSettlements(details.email_Id).then((userTransactions) => {
                    if (userTransactions) {
                        //console.log('success', user)
    ...
    return validateRequest(details)
        .then((response) => {
            if (response == true) {
                ...
                return userDao.updateProfile(query, updateDetails).then((userUpdated) => {

                    if (userUpdated) {
                       
                        return mapper.responseMappingWithData(usrConst.CODE.Success, usrConst.MESSAGE.Success, userUpdated.isBanned)
...
async function getTotalVolumeMerchantCheck(details) {
    return validateRequest(details)
        .then((response) => {
            if (response == true) {
                return dao.getTotalVolumeMerchant(details.start_date,details.end_date,details.status,details.email_Id).then((userTransactions) => {
                    if (userTransactions) {
                        //console.log('success', user)
            ...
            if (response == true) {
                return dao.getAllGateways().then((userGateways) => {
                    if (userGateways) {
                        ...
                        return mapper.responseMappingWithData(usrConst.CODE.Success, usrConst.MESSAGE.Success, userGateways)
    ...
    return validateRequest(details)
        .then(async (response) => {
            if (response == true) {
                ...
                if(gatewayInfo.balance<details.amount) return mapper.responseMappingWithData(usrConst.CODE.BadRequest, usrConst.MESSAGE.internalServerError, 'Low Balance')
                const timeElapsed = Date.now();
                const today = new Date(timeElapsed);
                let updateDetails = {
                    gatewayName:details.gatewayName,
                ...
                await dao.updateGatewaySettlementsAndBalance(details.gatewayName,gatewayUpdate)

                return dao.updateGatewaySettlements(query,updateDetails).then((user) => {
                    if (user) {
                        //console.log('success', user)

                        return mapper.responseMappingWithData(usrConst.CODE.Success, usrConst.MESSAGE.Success, user.gatewaySettlements)
...
async function getAllGatewaysSettlements(details) {
    return validateRequest(details)
        .then((response) => {
            if (response == true) {
                return dao.getAllGatewaysSettlements(details).then((userGatewaySettlements) => {
                    if (userGatewaySettlements) {
                        //console.log('success', user)

                        return mapper.responseMappingWithData(usrConst.CODE.Success, usrConst.MESSAGE.Success, userGatewaySettlements)
...
async function setGatewaySwitch(details) {
    return validateRequest(details)
        .then((response) => {
            if (response == true) {
                return dao.updateGatewaySwitch(details.gatewayName,details).then((updated) => {
                    if (updated) {
