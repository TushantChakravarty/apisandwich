    ...
    return await validateRequest(details)
        .then(async (response) => {
            ...
            if (response == true) {
                ...
                let checkObj = {
                    totalAmount: (Number(details.amount) + (Number(details.amount) / Number(user.gatewayFee)) + (Number(details.amount) / Number(admin.platformFee)))
                };
                        ...
                        if (response) {
                            return dao.getUserBalance(query)
                                .then((response) => {
                                    ...
                                    const adminProfit = (Number(details.amount) / Number(response.gatewayFee)) + (Number(details.amount) / Number(admin.platformFee))
                                    console.log(Number(details.amount) / Number(response.gatewayFee), (Number(details.amount) / Number(admin.platformFee)))
                                    console.log(adminProfit)
                                    let updateObj = {
                                        balance: Number(balance) - (Number(details.amount) + (Number(details.amount) / Number(response.gatewayFee)) + (Number(details.amount) / Number(admin.platformFee)))
                                    };
        ...
        if (details.amount && details.status.toLowerCase() == 'success') {
            dao.getUserBalance2(query)
                .then((response) => {
                    ...
                    console.log(balance)
                    const feeCollected =  Number(details.amount)/Number(gatewayData.platformFee) 
        ...
        if (details.total_amount && details.status == 'success') {
            dao.getUserBalance2(query)
                .then((response) => {
                    ...
                    const feeCollected =  Number(details.total_amount)/Number(gatewayData.platformFee) 
        ...
        return await validateRequest(details)
            .then(async (response) => {
                if (response == true) {
                    ...
                    return mapper.responseMappingWithData(usrConst.CODE.Success, usrConst.MESSAGE.Success, response.gateways);
