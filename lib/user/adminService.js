...
async function saveTxAirpay(details) {
    try {
        ...
        // Enqueue the job
        await airpayQueue.createJob(jobData).save();
...
// Process the queue
airpayQueue.process(async (job) => {
    const { query, updateObj, details, transaction, admin, gatewayData } = job.data;
    ...
    try {
        ...
        return 'success'; // This value will be returned to the promise
    } catch (error) {
        console.error(error);
        return 'error';
    }
