...
async function getAllTransactions(skip, limit) {
  ...
  async function getTransactionsSummaryYesterday() {
    try {
      ...
      const istStartDate = moment().tz('Asia/Kolkata').subtract(1, 'days').startOf('day').toISOString();
      const istEndDate = moment().tz('Asia/Kolkata').subtract(1, 'days').endOf('day').toISOString();
