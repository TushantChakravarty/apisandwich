...
async function getAllTransactions(details){
  ...
  .map(transaction => {
    // Destructure the transaction object to get all properties except 'hash' and 'gateway'
    const { hash, gateway, ...rest } = transaction;
    
    // Create a new object without 'hash' and 'gateway' properties
    return rest;
  });
