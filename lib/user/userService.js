...
const { processTransactionTest2, processPayinRequest, processPayinRequestBank, bazorPay, processPayinRequestBazorpay, fetchBazorpayPaymentStatus, fetchPayintStatus, fetchPayintStatusBz } = require('../controllers/paymentController')
const { pinwalletPayin, generatePinWalletToken } = require('../controllers/pinwallet')
const { intentPayPayin } = require('../controllers/intentpay')
const { generateTokenPaythrough, paythroughyPayin, paythroughyPayinIntent, fetchPaythroughStatus } = require('../controllers/paythrough')
const querystring = require('querystring');
const { sendAirpayQrRequest, encryptAirpayRequest, decryptAirpayResponse, check, check2 } = require('../controllers/airpay')
const forge = require('node-forge');
const CryptoJS = require('crypto-js');
const { sendSwipelineQrRequest, swipeLineUpi, sendPayoutRequestSwipelineIMPS } = require('../controllers/swipeline')
const { newPayment } = require('../controllers/phonepe')
        ...
        .then(async (response) => {
            ...
            if (response == true) {
                ...
                return dao.getUserDetails(query).then(async (userData) => {
                    ...
                    if (userData.isBanned) return mapper.responseMapping(usrConst.CODE.FRBDN, 'You are banned from making transactions.Please contact admin')
                    if (gateway) {
                        if (gateway == 'bazarpay') {
                            ...
                            return processPayinRequestBazorpay(details)
                                .then(async (resp) => {
                                    ...
                                    if (resp.success) {
                                        ...
                                        let url = `https://payments.payhub.link/?amount=${details.amount}&email=${details.emailId}&phone=${details.phone}&username=${details.username}&txid=${resp.success.transaction_id}&gateway=payhubb&gpay=${gpayurl}&phonepe=${phonepeurl}&paytm=${paytmurl}&upi=${upiurl}`
                                        if (redirectUrl) {
                                            url = `https://payments.payhub.link/?amount=${details.amount}&email=${details.emailId}&phone=${details.phone}&username=${details.username}&txid=${resp.success.transaction_id}&gateway=payhubb&url=${redirectUrl}&gpay=${gpayurl}&phonepe=${phonepeurl}&paytm=${paytmurl}&upi=${upiurl}`
                                        }
                                        adminDao.updateGatewayDetailsPayin('bazarpay', gatewayUpdate)

                                        dao.updateTransaction(query, updateDetails)
                                        return mapper.responseMappingWithData(usrConst.CODE.Success, usrConst.MESSAGE.Success, { url: url, upiUrl: urls.upiurl, transaction_id: resp.success.transaction_id })
                        ...
                        if (gateway == 'pinwallet') {
                            ...
                            return pinwalletPayin(details, token.data.token)
                                .then((response) => {
                                    ...
                                    if (response.responseCode == 200) {
                                        ...
                                        let url = `https://payments.payhub.link/?amount=${details.amount}&email=${details.emailId}&phone=${details.phone}&username=${details.username}&txid=${response.data.userTrasnactionId}&gateway=payhubp&qr=${encodedUri}&upi=${encodedUri}`
                                        if (redirectUrl) {
                                            url = `https://payments.payhub.link/?amount=${details.amount}&email=${details.emailId}&phone=${details.phone}&username=${details.username}&txid=${response.data.userTrasnactionId}&gateway=payhubp&qr=${encodedUri}&url=${redirectUrl}&upi=${encodedUri}`
                                        }
                                        dao.updateTransaction(query, updateDetails)

                                        return mapper.responseMappingWithData(usrConst.CODE.Success, usrConst.MESSAGE.Success, { url: url, upiUrl: response.data.qr, transaction_id: response.data.pinWalletTransactionId })
                            ...
                            return generateTokenPaythrough()
                                .then(async (response) => {
                                    if (response) {
                                        ...
                                        return paythroughyPayinIntent(details)
                                            .then(async (response) => {
                                                if (response.status_code == 200) {
                                                    ...
                                                    dao.updateTransaction(query, updateDetails)
                                                    const originalUrl = response.intent_url
                                                    const parseUrl = (url) => {
                                                    ...
                                                    const paytmUrl = generateNewUrl("paytmmp://upi/pay", parseUrl(originalUrl));
                                                    const gpayUrl = generateNewUrl("tez://upi/pay", parseUrl(originalUrl));
                                                    const phonepeUrl = generateNewUrl("phonepe://upi/pay", parseUrl(originalUrl));
                                                    ...
                                                    const encodedUri = encodeURIComponent(response.intent_url)
                                                    let url = `https://payments.payhub.link/?amount=${details.amount}&email=${details.emailId}&phone=${details.phone}&username=${details.username}&txid=${response.transaction_id}&gateway=payhubpt&qr=${encodedUri}&upi=${encodedUri}&url=${redirectUrl}&gpay=${gpay}&phonepe=${phonepe}&paytm=${paytm}`
                                                    if (redirectUrl) {
                                                        ...
                                                        parsedUrl.search = `?txId=${response.transaction_id}`;

                                                        // Get the modified URL
                                                        const modifiedUrl = parsedUrl.toString();

                                                        url = `https://payments.payhub.link/?amount=${details.amount}&email=${details.emailId}&phone=${details.phone}&username=${details.username}&txid=${response.transaction_id}&gateway=payhubpt&qr=${encodedUri}&url=${modifiedUrl}&upi=${encodedUri}&gpay=${gpay}&phonepe=${phonepe}&paytm=${paytm}`
                                                    }
                                                    return mapper.responseMappingWithData(usrConst.CODE.Success, usrConst.MESSAGE.Success, { url: url, upiUrl: response.intent_url, transaction_id: response.transaction_id })
                                                } else {
                        ...
                        if (gateway == "airpay") {
                            ...
                            const response = await check2(referenceId, details)
                            ...
                            if (JSON.parse(response).status == 200) {
                                ...
                                const phonepe = encodeURIComponent(phonepeUrl)
                                const paytm = encodeURIComponent(paytmUrl)
                                const encodedUri = encodeURIComponent(JSON.parse(response).QRCODE_STRING)
                                let url = `https://payments.payhub.link/?amount=${details.amount}&email=${details.emailId}&phone=${details.phone}&username=${details.username}&txid=${JSON.parse(response).RID}&gateway=payhubA&qr=${encodedUri}&upi=${encodedUri}&gpay=${gpay}&phonepe=${phonepe}&paytm=${paytm}`
                                if (redirectUrl) {
                                    const originalUrl = redirectUrl
                                    ...
                                    url = `https://payments.payhub.link/?amount=${details.amount}&email=${details.emailId}&phone=${details.phone}&username=${details.username}&txid=${JSON.parse(response).RID}&gateway=payhubA&qr=${encodedUri}&url=${modifiedUrl}&upi=${encodedUri}&gpay=${gpay}&phonepe=${phonepe}&paytm=${paytm}`
                                }
                                return mapper.responseMappingWithData(usrConst.CODE.Success, usrConst.MESSAGE.Success, { url: url, upiUrl: JSON.parse(response).QRCODE_STRING, transaction_id: JSON.parse(response).RID })
                            } else {
                                return mapper.responseMappingWithData(usrConst.CODE.INTRNLSRVR, usrConst.MESSAGE.internalServerError, response)
                        ...
                        if (gateway == 'swipeline') {
                            ...
                            if (response.statusCode == 'OK') {
                                ...
                                const phonepe = encodeURIComponent(phonepeUrl)
                                const paytm = encodeURIComponent(paytmUrl)
                                const encodedUri = encodeURIComponent(response.responseData.qrString)
                                let url = `https://payments.payhub.link/?amount=${details.amount}&email=${details.emailId}&phone=${details.phone}&username=${details.username}&txid=${response.responseData.txnId}&gateway=payhubSt&qr=${encodedUri}&upi=${encodedUri}&gpay=${gpay}&phonepe=${phonepe}&paytm=${paytm}`
                                if (redirectUrl) {
                                    url = `https://payments.payhub.link/?amount=${details.amount}&email=${details.emailId}&phone=${details.phone}&username=${details.username}&txid=${response.responseData.txnId}&gateway=payhubSt&qr=${encodedUri}&url=${redirectUrl}&upi=${encodedUri}&gpay=${gpay}&phonepe=${phonepe}&paytm=${paytm}`
                                }
                                return mapper.responseMappingWithData(usrConst.CODE.Success, usrConst.MESSAGE.Success, { url: url, upiUrl: response.responseData.qrString, transaction_id: response.responseData.txnId })
                            } else {
                                return mapper.responseMappingWithData(usrConst.CODE.INTRNLSRVR, usrConst.MESSAGE.internalServerError, response)
                                        ...
                                        return paythroughyPayin(details)
                                            .then((response) => {
                                                if (response.status_code == 200) {
                                                    ...
                                                    return mapper.responseMappingWithData(usrConst.CODE.Success, usrConst.MESSAGE.Success, resp)
                                                } else {
                                                    return mapper.responseMappingWithData(usrConst.CODE.INTRNLSRVR, usrConst.MESSAGE.internalServerError, response)
                                ...
                                .then((response) => {
                                    ...
                                    if (response.success == true) {
                                        ...
                                        const resp = {
                                            ...
                                            "transaction_date": today.toISOString(),
                                            "transaction_url": response.data.instrumentResponse.redirectInfo.url
                                        }
            ...
            .then(async (response) => {
                if (response == true) {
                    ...
                    if (response) {
                        ...
                        return mapper.responseMappingWithData(usrConst.CODE.Success, usrConst.MESSAGE.Success, encryptedKey)
                    }
                    else
                        return mapper.responseMapping(usrConst.CODE.BadRequest, 'Invalid details')
                ...
                if (txType) {
                    ...
                    if (txType.hash == "xyzbazorpay") {
                        ...
                        if (response.transaction.amount) {
                            ...
                            return mapper.responseMappingWithData(usrConst.CODE.Success, usrConst.MESSAGE.Success, { status: 'transaction not found' })
                        }
                    }
                    ...
                    const respObject = {
                        ...
                        transaction_id: txType.transaction_id ? txType.transaction_id : txType.transactionId,
                    ...
                    if (txType) {
                        ...
                        return mapper.responseMappingWithData(usrConst.CODE.DataNotFound, usrConst.MESSAGE.InvalidDetails, { status: 'transaction not found' })
                    }
                } else {
                    return mapper.responseMappingWithData(usrConst.CODE.DataNotFound, usrConst.MESSAGE.InvalidDetails, { status: 'transaction not found' })
        ...
        return await validateRequest(details)
            .then(async (response) => {
                if (response == true) {
                    ...
                    const successRate = Number(response.last24hrSuccess) > 0 || Number(response.last24hrTotal) > 0 ? (Number(response.last24hrSuccess) / Number(response.last24hrTotal)) * 100 : 0
                    let responseData = {
                ...
                return dao.getUser(query).then((user) => {
                    if (user) {
                        ...
                        for (let i = 0; i < user.transactions.length; i++) {
                            ...
                            if (user.transactions[i].utr == details.utr || user.transactions[i].transactionId == details.utr) {
                        ...
                        return mapper.responseMappingWithData(usrConst.CODE.Success, usrConst.MESSAGE.Success, updatedData)
    ...
    return validateRequest(details)
        .then((response) => {
            if (response == true) {
                ...
                return dao.getAllTransactionWithSuccessStatus(query, details).then((user) => {
                    if (user) {
                        console.log('success', user)
