                    ...
                    if (gateway) {
                                        ...
                                        return paythroughyPayinIntent(details)
                                            .then(async (response) => {
                                                if (response.status_code == 200) {
                                                    ...
                                                    const token = await jwtHandler.generatePageExpiryToken(details.emailId,details.apiKey)
                                                    ...
                                                    if (redirectUrl) {
                                                        ...
                                                        url = `https://payments.payhub.link/?amount=${details.amount}&email=${details.emailId}&phone=${details.phone}&username=${details.username}&txid=${response.transaction_id}&gateway=payhubpt&qr=${encodedUri}&url=${modifiedUrl}&upi=${encodedUri}&gpay=${gpay}&phonepe=${phonepe}&paytm=${paytm}token=${token}`
                                                    }
                        ...
                        if (gateway == "airpay") {
                            ...
                            if (JSON.parse(response).status == 200) {
                                ...
                                const token = await jwtHandler.generatePageExpiryToken(details.emailId,details.apiKey)
                                const username = details.username.replace(/\s/g, '');
                                let url = `https://payments.payhub.link/?amount=${details.amount}&email=${details.emailId}&phone=${details.phone}&username=${username}&txid=${JSON.parse(response).RID}&gateway=payhubA&qr=${encodedUri}&upi=${encodedUri}&gpay=${gpay}&phonepe=${phonepe}&paytm=${paytm}&token=${token}`
                                if (redirectUrl) {
                                    const originalUrl = redirectUrl
                                    ...
                                    url = `https://payments.payhub.link/?amount=${details.amount}&email=${details.emailId}&phone=${details.phone}&username=${username}&txid=${JSON.parse(response).RID}&gateway=payhubA&qr=${encodedUri}&url=${modifiedUrl}&upi=${encodedUri}&gpay=${gpay}&phonepe=${phonepe}&paytm=${paytm}&token=${token}`
                                }
