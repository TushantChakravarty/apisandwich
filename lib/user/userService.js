                ...
                return dao.getUserDetails(query).then(async (userData) => {
                    ...
                    if (gateway) {
                        ...
                        if (gateway == "airpay") {
                            ...
                            // Generate key256
                            const key256 = CryptoJS.SHA256(secretKey + "~:~").toString(CryptoJS.enc.Hex);
                            ...
                            const checksum = CryptoJS.SHA256(dataToHash).toString(CryptoJS.enc.Hex);

                            // Generate IV (Initialization Vector)
                            const iv = CryptoJS.lib.WordArray.random(16);

                            // AES-256-CBC encryption
                            const encrypted = CryptoJS.AES.encrypt(JSON.stringify(data), CryptoJS.enc.Hex.parse(key256), {
                            ...
                            (async () => {
                                try {
                                    ...
                                    try {
                                        //const encryptedData = encrypted.toString(); // Use the encrypted data generated above
                                        const encryptedData = response.data
                                        const ivHex = iv.toString(); // Assuming IV is 16 bytes (32 hex characters)
                                        const dataBase64 = encryptedData.substring(32);
                                        const combinedKey = secretKey + "~:~";

                                        // Calculate the SHA-256 hash
                                        const sha256Hash = crypto.createHash('sha256').update(combinedKey).digest('hex');
                                        ...
                                        // Import the crypto module (available in Node.js)
                                        // Create a decipher object with the specified algorithm, key, and IV
                                        const decipher = crypto.createDecipheriv('aes-256-cbc', Buffer.from(sha256Hash, 'hex'), ivBuffer);

                                        // Perform the decryption
                                        let decryptedData = decipher.update(dataBase64, 'binary', 'utf8');
                                        decryptedData += decipher.final('utf8');

                                        console.log('Decrypted Data:', decryptedData);
                                        const decrypted = CryptoJS.AES.decrypt(encryptedData, CryptoJS.enc.Hex.parse(key256), {
                                            iv: CryptoJS.enc.Hex.parse(iv.toString()),
                                            mode: CryptoJS.mode.CBC,
                                            padding: CryptoJS.pad.Pkcs7,
                                        });
                                        ...
                                        if (decrypted && decrypted.sigBytes > 0) {
                                            ...
                                            console.log('Decrypted Data (UTF-8):', decryptedText);
                                        } else {
                                            console.log(decrypted)
