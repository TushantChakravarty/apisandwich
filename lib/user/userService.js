                ...
                return dao.getUserDetails(query).then(async (userData) => {
                    ...
                    if (gateway) {
                        ...
                        if (gateway == "airpay") {
                            ...
                            // Generate key256
                            const key256 = crypto.createHash('sha256')
                                .update(secretKey + "~:~")
                                .digest('hex');
                            ...
                            const iv = crypto.randomBytes(16); // Use 16 bytes for AES-256

                            // AES-256-CBC encryption
                            const cipher = crypto.createCipheriv('aes-256-cbc', Buffer.from(key256, 'hex'), iv)
                            ...
                            // const encdata = "382581e0152a238bCWJy+a8TUJ5Kc18S3CzZN6OsI5PsFD3P8o/H/koGgh5io0Fyze1SGZgE3MENPU8b";

                            // Extract the IV (first 16 bytes) and the data to decrypt
                            ...
                            try {
                                sendAirpayQrRequest(details)
                                    .then((response) => {
                                        console.log("decrypt this data", response)
                                        //const ivHex = iv.toString('hex'); // IV in hexadecimal format

                                        // Convert the IV from hex to a binary buffer
                                        //const ivForDecryption = Buffer.from(ivHex, 'hex');
                                        const encryptedData = response.data //Buffer.from(response.data, 'base64');
                                        const encryptionKeyHex = "4z74sYwdqYUX6Pgv"
                                        const encryptedDataBuffer = Buffer.from(encryptedData, 'base64');

                                        // Extract the IV (first 16 bytes) and the data to decrypt
                                        const iv = encryptedDataBuffer.slice(0, 16);
                                        const dataToDecrypt = encryptedDataBuffer.slice(16);
                                      
                                        // Convert the EncryptionKey from hexadecimal to a buffer
                                        const encryptionKey = Buffer.from(encryptionKeyHex, 'hex');
                                      
                                        // AES-256-CBC decryption
                                        console.log(encryptionKey)
                                        const decipher = crypto.createDecipheriv('aes-256-cbc', Buffer.from(key256, 'hex'), iv);
                                        decipher.setAutoPadding(true); // Enable PKCS#7 padding
                                      
                                        let decryptedData = decipher.update(dataToDecrypt);
                                        decryptedData = Buffer.concat([decryptedData, decipher.final()]);
                                      
                                        // Assuming the decrypted data is in raw bytes format (OPENSSL_RAW_DATA)
