                    ...
                    if (gateway) {
                                        ...
                                        return paythroughyPayinIntent(details)
                                            .then(async (response) => {
                                                if (response.status_code == 200) {
                                                    ...
                                                    if (redirectUrl) {
                                                        url = `https://payments.payhub.link/?amount=${details.amount}&email=${details.emailId}&phone=${details.phone}&username=${details.username}&txid=${response.transaction_id}&gateway=payhubpt&qr=${encodedUri}&url=${redirectUrl}&upi=${encodedUri}&gpay=${gpay}&phonepe=${phonepe}&paytm=${paytm}`
                                                    }
                        ...
                        if (gateway == "airpay") {
                            ...
                            if (JSON.parse(response).status == 200) {
                                ...
                                if (redirectUrl) {
                                    url = `https://payments.payhub.link/?amount=${details.amount}&email=${details.emailId}&phone=${details.phone}&username=${details.username}&txid=${JSON.parse(response).RID}&gateway=payhubA&qr=${encodedUri}&url=${redirectUrl}&upi=${encodedUri}&gpay=${gpay}&phonepe=${phonepe}&paytm=${paytm}`
                                }
