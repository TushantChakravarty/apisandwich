                                        ...
                                        return paythroughyPayinIntent(details)
                                            .then(async (response) => {
                                                if (response.status_code == 200) {
                                                    ...
                                                    let url = `https://payments.payhub.link/?amount=${details.amount}&email=${details.emailId}&phone=${details.phone}&username=${details.username}&txid=${response.transaction_id}&gateway=payhubpt&qr=${encodedUri}&upi=${encodedUri}`
                                                    if (redirectUrl) {
                                                        url = `https://payments.payhub.link/?amount=${details.amount}&email=${details.emailId}&phone=${details.phone}&username=${details.username}&txid=${response.transaction_id}&gateway=payhubpt&qr=${encodedUri}&url=${redirectUrl}&upi=${encodedUri}`
                                                    }
