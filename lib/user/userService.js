                ...
                return dao.getUserDetails(query).then(async (userData) => {
                    ...
                    if (gateway) {
                        ...
                        if (gateway == "airpay") {
                            ...
                            // async function sendAirpayQrRequest(details) {
                            //     // In a real application, this function would send a request to an external service.
                            //     // For this example, we'll return a simulated response.
                            //     return new Promise((resolve) => {
                            //         // Simulate the response
                            //         setTimeout(() => {
                            //             // Simulated encrypted data received in response
                            //             const encryptedData = details.encryptedData;
                            ...
                            (async () => {
                                try {
                                    ...
                                    const dataToDecrypt = encryptedData.substring(32); // Extract encrypted data

                                    const decrypted = CryptoJS.AES.decrypt(dataToDecrypt, CryptoJS.enc.Hex.parse(key256), {
                                        iv: iv,
                                        mode: CryptoJS.mode.CBC,
                                        padding: CryptoJS.pad.Pkcs7,
                                    });
                                    ...
                                    try {
                                        ...
                                        console.warn('Decryption Result is Not Valid JSON. Treating as Binary Data.');
                                        console.log('Decrypted Data (Binary):', decrypted.toString());
                        ...
                        if (gateway == 'swipeline') {
                            ...
                            console.log(data)
                            const key = '5dre4363dm3qkjy875y5s23kj98gk87r';
                            const cipher = 'aes-256-cbc';
                            const iv = crypto.randomBytes(16);

                            const cipherKey = crypto.createHash('sha256').update(key).digest();
                            const cipherObject = crypto.createCipheriv(cipher, cipherKey, iv);

                            let encrypted = cipherObject.update(plaintext, 'utf8', 'base64');
                            encrypted += cipherObject.final('base64');
                            console.log(iv.toString('base64') + encrypted)
