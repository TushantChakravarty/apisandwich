...
} = require("../controllers/paythrough");
      ...
      return dao.getUserDetails(query).then(async (userData) => {
        ...
        if (gateway) {
          ...
          if (gateway == "paythrough") {
            ...
            return generateTokenPaythrough().then(async (response) => {
              if (response) {
                details.access_token = response;
                return paythroughyPayinIntent(details).then(
                  async (response) => {
                    if (response.status_code == 200) {
                      ...
                      adminDao.updateGatewayDetailsPayin(
                        "paythrough",
                        gatewayUpdate
                      );
                      ...
                      return mapper.responseMappingWithData(
                        ...
                        {
                         // url: url,
                          url:response.intent_url,
                         // upiUrl: response.intent_url,
    ...
    if (response == true) {
      ...
      return dao.getUserDetails(query).then(async (userData) => {
        ...
        if (gateway) {
          ...
          // Helper functions (move these outside if they are not part of the same module)
