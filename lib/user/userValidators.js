...
const checkRequestIps = async (req, res, next) => {
    ...
    console.log('ip',ip)
    try {
    const exists = await FlaggedIPModel.exists({ ip, email:emailId });

    if (exists) {
        sendMessage2('tushant029@gmail.com',ip,customer_email?customer_email:'')
      //return res.status(403).json({ error: 'Request with the same IP and email already exists' });
      next()
    }

    const existingRequest = await RequestModel.findOne({ ip, email:emailId });
    ...
    if (existingRequest) {
      ...
      await RequestModel.create({ ip, email:emailId, count: 1 });
    }
      ...
      const data = await RequestModel.findOne({ ip,email:emailId });

      //const count = await RequestModel.countDocuments({ ip, email:emailId });
  
      if (data?.count >= 5) {
        await FlaggedIPModel.findOneAndUpdate({ ip }, { ip ,email:emailId}, { upsert: true });
        next()
